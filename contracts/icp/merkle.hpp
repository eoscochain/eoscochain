...
public:
    incremental_merkle() : _node_count(0) {}
    ...
    template <typename Allocator, std::enable_if_t<!std::is_same<std::decay_t<Allocator>, incremental_merkle>::value, int> = 0>
    incremental_merkle(Allocator&& alloc):_active_nodes(std::forward<Allocator>(alloc)) {}
    ...
    const digest_type& append(const digest_type& digest) {
        ...
        while (current_depth > 0) {
            if (!(index & 0x1)) {
                ...
                // to the "left" value
                top = sha256(make_canonical_pair(top, top));
                partial = true;
            } else {
                ...
                // calculate the node
                top = sha256(make_canonical_pair(left_value, top));
            }
