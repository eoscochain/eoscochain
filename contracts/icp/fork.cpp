...
fork_store::fork_store(account_name code)
    : _code(code),
      ...
      _blocks(code, code),
      _producer_schedule(code, code),
      _pending_schedule(code, code),
      _store_parameters(code, code)
{
    if (!_store_parameters.exists()) {
        setmaxblocks(2 * 60 * 60); // default store blocks max one hour
    }
}

void fork_store::setmaxblocks(uint32_t max) {
    _store_parameters.set(store_parameters{max}, _code);
}
...
void fork_store::add_block_header_with_merkle_path(const block_header_state& h, const vector<block_id_type>& merkle_path) {
    ...
    if (producer_scheduler_changed) {
        ...
        eosio_assert(h.dpos_irreversible_blocknum <= h.pending_schedule_lib_num + 12, "changed producer schedule before irreversible"); // TODO: 12?

        update_producer_schedule(h.active_schedule);
    }
    ...
    if (!merkle_path.empty()) {
        auto mroot = get_block_mroot(merkle_path.front());
        for (auto it = merkle_path.cbegin() + 1; it != merkle_path.cend(); ++it) {
            mroot.append(*it);
    ...
    _block_states.emplace(_code, [&](auto& b) {
        ...
        b.id = h.id;
        b.block_num = h.block_num;
        b.blockroot_merkle = h.blockroot_merkle;
    })
}
...
void fork_store::cutdown(uint32_t num) {
    auto head = *_block_states.get_index<N(libblocknum)>().begin();
    eosio_assert(num <= head.last_irreversible_blocknum(), "block number not irreversible");
...
void fork_store::add_block_id(const block_id_type& block_id) {
    auto by_blockid = _blocks.get_index<N(blockid)>();
...
bool fork_store::is_producer(account_name name, const ::public_key& key) {
    auto schedule = _producer_schedule.get();
